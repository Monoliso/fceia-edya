 ...
 */
int ghashtable_nelems(TablaHash tabla) { return tabla->numElems; }

/**
 * Retorna la capacidad de la tabla.
 */
int ghashtable_capacidad(TablaHash tabla) { return tabla->capacidad; }
...
void ghashtable_destruir(TablaHash tabla) {
  ...
  for (unsigned idx = 0; idx < tabla->capacidad; ++idx)
    if (tabla->elems[idx].dato != NULL) tabla->destr(tabla->elems[idx].dato);
...
void *ghashtable_buscar(TablaHash tabla, void *dato) {
  ...
  if (tabla->elems[idx].dato == NULL) return NULL;
...
void ghashtable_eliminar(TablaHash tabla, void *dato) {
  ...
  if (tabla->elems[idx].dato == NULL) return;
