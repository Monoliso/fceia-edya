...
int main() {
  ...
  // Iniciar tabla hash
  TablaHash tabla = ghashtable_crear(
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    contacto_imprimir(agenda[i]);
    puts("");
    Contacto *ret = ghashtable_buscar(tabla, agenda[i]);
    if (ret != NULL)
  ...
  for (int i = 5; i > 2; --i) {
    ...
    contacto_imprimir(agenda[i]);
    puts("");
    ghashtable_eliminar(tabla, agenda[i]);
  }
  ...
  puts("");
  ghashtable_insertar(tabla, nuevoContacto);
  // Chequeamos que se haya sobrescrito
  Contacto *ret = ghashtable_buscar(
